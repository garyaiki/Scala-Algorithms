...
class DigraphSuite extends FlatSpec {
  ...
  it should "find vertex adjacencies" in new DigraphBuilder {
    assert(tinyDAG.adj.flatten.corresponds(List(5,1,6,0,3,5,4,9,4,6,7,11,10,12,12))(equals))
    assert(tinyDG.adj.flatten.corresponds(List(5,1,0,3,5,2,3,2,4,9,4,8,0,6,9,6,11,10,12,4,12,9))(equals))
  }
  
  it should "reverse and find vertex adjacencies" in new DigraphBuilder {
    ...
    assert(reversedDG.adj.flatten.corresponds(List(6,2,0,4,3,4,2,11,6,5,3,0,8,7,6,12,7,6,9,9,11,10))(equals))
  }
} 